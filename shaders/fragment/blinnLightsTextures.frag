...
{
    return abs(diff_dotProduct) * color;
}
...
vec4 pointLight(Light light, vec3 world_normal, vec4 world_position, vec3 viewDirection, vec4 ambient, vec4 texture_color)
{
...
vec4 directionalLight(Light light, vec3 world_normal, vec4 world_position, vec3 viewDirection, vec4 ambient, vec4 texture_color)
{
...
vec4 spotLight(Light light, vec3 world_normal, vec4 world_position, vec3 viewDirection, vec4 ambient, vec3 camera, vec4 texture_color)
{
...
void main() {
    ...
    vec4 texture_color = texture(textureUnitID, uvc); ///p�e�teten� texelu z texture (texturovac� jednotka, kter� se m� na��st | body, na kter� se to m� "aplikovat"
    ...
    {
        if(lights[i].type == 1)
        fragmentColor += pointLight(lights[i], world_normal, ex_worldPosition, viewDirection, ambient, texture_color);
        else if (lights[i].type == 2)
        {
            fragmentColor += spotLight(lights[i], world_normal, ex_worldPosition, viewDirection, ambient, camera, texture_color);
        }

        else if(lights[i].type == 3)
        fragmentColor += directionalLight(lights[i], world_normal, ex_worldPosition, viewDirection, ambient, texture_color);
    }
